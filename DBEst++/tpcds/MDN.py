...
class DenMDN:
    ...
    def fit(self,x_points, y_points, encoded=True,lr = 0.001,n_workers=0): 
        ...
        n_epoch = 20#self.config.config["n_epoch"]
    ...
    def update(self,x_points_update, y_points_update, x_points_transfer, y_points_transfer, encoded=True,lr = 0.001, n_epoch=10, alpha=0.15, n_workers=0, num_gauss=40, hid_neurons=2): 
...
def train(mode,name):
    ...
    t1 = time.time()
    d = Dataset.Data('./data/',train_file=None, test_file=None, update_path=None, transfer_set=None, x_attributes = ["ss_quantity"], y_attributes = ["ss_sales_price"], sep=',')    
    d.train_update_split(table='tpcds_for_MDN.csv', haveheader=True, permute=True, convert_to_timestamp=False, percentage=0.2,num_update_chunks=1)  
    ...
    x_values, y_values = d.read_data(d.train_file, haveheader=True)
...
def distilling(basemodel):
    ...
    x_values, y_values = model.dataset.read_data(model.dataset.train_file, haveheader=True)
    ...
    for num in range(1):
        ...
        x_values_transfer, y_values_transfer = model.dataset.read_data(model.dataset.transfer_set, haveheader=True)
...
def update_with_fusion(basemodel):
    ...
    x_values, y_values = model.dataset.read_data(model.dataset.train_file, haveheader=True)
    train_set = pd.DataFrame({**x_values, **y_values})
    transfer_data = Dataset.stratified_sample(train_set, model.dataset.x_attributes, size=100000, seed=123, keep_index= True) 
    transfer_data.to_csv(model.dataset.transfer_set, sep=model.dataset.delimiter, index=None)
    ...
    for num,batch in enumerate(update_batches):
        ...
        x_values_update, y_values_update = model.dataset.read_data(update_file, haveheader=True)
...
def retrain_all(basemodel):
    ...
    for i, file in enumerate(files):
        t1 = time.time()
        x_tmp, y_tmp = model.dataset.read_data('data/update_batches/'+file, haveheader=True)
        ...
        with open(basemodel.replace('.dill',str(i+1).zfill(2)+'.dill'),'wb') as dum:
            dill.dump(model,dum)
...
if __name__=="__main__":
    ...
    #train(mode="train", name='DenMDN_tpcds.dill')
    #distilling(basemodel='DenMDN_census.dill')
    #distill_musample(basemodel='DenMDN_census.dill')
    #update_with_fusion(basemodel="DenMDN_tpcds.dill")
    #update_with_finetune(basemodel="DenMDN_tpcds.dill")
    #update_FTs(basemodel="DenMDN_tpcds.dill")
    retrain_all(basemodel="DenMDN_tpcds.dill")
