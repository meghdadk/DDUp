...
class DenMDN:
    ...
    def update(self,x_points_update, y_points_update, x_points_transfer, y_points_transfer, encoded=True,lr = 0.0001, n_epoch=20, alpha=0.16, n_workers=0, num_gauss=40, hid_neurons=2): 
    ...
    def finetune(self,x_points, y_points, encoded=True,lr = 0.000001,n_workers=0):
...
def prepare_data():
    d = Dataset.Data('./data/',train_file=None, test_file=None, update_path=None, transfer_set=None,
                     x_attributes = ["slope"], y_attributes = ["elevation"], sep=',')

    d.train_update_split(table='forest_for_MDN.csv', haveheader=True, permute=True,
                         convert_to_timestamp=False, percentage=1,num_update_chunks=1)  
    d.train_update_split(table='forest_for_MDN.csv', haveheader=True, permute=False,
                         convert_to_timestamp=False, percentage=1,num_update_chunks=1) 
...
def train(mode,name):
    ...
    #d.train_update_split(table='DMV_for_MDN.csv', haveheader=True, permute=True, convert_to_timestamp=False, percentage=0.2,num_update_chunks=1)  
...
def update_with_fusion(basemodel, tr_size):
    if not os.path.exists(basemodel):
        print ("Model not found!")
        return
    ...
    x_values, y_values = model.dataset.read_data(model.dataset.train_file, haveheader=True, to_dict=True)
    train_set = pd.DataFrame({**x_values, **y_values})
    transfer_data = Dataset.stratified_sample(train_set, model.dataset.x_attributes, size=tr_size, seed=123, keep_index= True) 
    transfer_data.to_csv(model.dataset.transfer_set, sep=model.dataset.delimiter, index=None)
    ...
    for num,batch in enumerate(update_batches):
        ...
        if num%15 == 0:
            df_tr_new = df_tr_new.sample(n=tr_size)
...
if __name__=="__main__":
    ...
    #update_with_fusion(basemodel="base.dill", tr_size=110000)
