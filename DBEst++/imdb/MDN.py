...
class DenMDN:
    ...
    def fit(self,x_points, y_points, encoded=True,lr = 0.0001, n_workers=0): 

        n_epoch = 30#self.config.config["n_epoch"]
        n_gaussians = 1024#self.config.config["n_gaussians_reg"]
        ...
        for epoch in range(n_epoch):
            ...
            # train the model
            for i, (minibatch, labels) in enumerate(my_dataloader): 
    ...
    def update(self,x_points_update, y_points_update, x_points_transfer, y_points_transfer, encoded=True,lr = 0.0001, alpha=0.5, gamma=0.5, n_workers=0, bs=1024): 
        ...
        for epoch in range(n_epoch):
            ...
            for (minibatch_update, labels_update), (minibatch_transfer, labels_transfer) in zip(my_dataloader_update, cycle(my_dataloader_transfer)):
                ...
                minibatch_transfer = minibatch_transfer.to(device)
                labels_transfer = labels_transfer.to(device)

                minibatch_update = minibatch_update.to(device)
                labels_update = labels_update.to(device)
                ...
                loss =  alpha*(loss1) + (1-alpha)*(gamma*loss2+(1-gamma)*loss3)
    ...
    def finetune(self,x_points, y_points, encoded=True,lr = 0.0001,n_workers=0):
...
def create_ref():
    t1 = time.time()
    d = Dataset.Data('../../job_data/',train_file=None, test_file=None, update_path=None, transfer_set=None, x_attributes = ["info_type_id"], y_attributes = ["production_year"], sep=',')
    d.update_path = "../../job_data/"
    x_values, y_values = d.read_data("../../job_data/title_until_part5.csv", haveheader=True, use_cols=['production_year', 'info_type_id'],to_dict=True)
    ...
    with open('reference.dill','wb') as dum:
        dill.dump(MDN,dum)


def train(mode, ref, name):
    ...
    t1 = time.time()
    if ref is None:
        d = Dataset.Data('./data/',train_file=None, test_file=None, update_path=None, transfer_set=None, x_attributes = ["info_type_id"], y_attributes = ["production_year"], sep=',')    
        d.train_update_split(table='imdb_for_MDN.csv', haveheader=True, permute=True, convert_to_timestamp=False, percentage=0.2,num_update_chunks=1)  
    ...
    x_values, y_values = d.read_data("../../job_data/title_until_part1.csv", haveheader=True, use_cols=['production_year', 'info_type_id'], to_dict=True)
    d.create_frequency_tables(data=x_values)
...
def update_with_fusion(basemodel, tr_size, cols):
    if not os.path.exists(basemodel):
        print ("Model not found!")
        return

    print ("Start preparing data ...")
    update_batches = ['title_only_part2.csv','title_only_part3.csv','title_only_part4.csv','title_only_part5.csv']
    tr_sets = ['title_until_part1.csv','title_until_part2.csv','title_until_part3.csv','title_until_part4.csv']
    alphas = [0.5, 0.32, 0.16, 0.09]
    tr_sizes = [100000, 200000, 300000, 400000]
    update_batches = ['title_only_part2.csv','title_only_part3.csv','title_only_part4.csv','title_only_part5.csv']
    tr_sets = ['title_until_part1.csv','title_until_part2.csv','title_until_part3.csv','title_until_part4.csv']
    alphas = [0.5, 0.32, 0.16, 0.09]
    gammas = [1/2, 3/4, 5/6, 5/6]
    bs = [1024, 1024, 1024, 1024]
    tr_sizes = [100000, 300000, 350000, 450000]
    ...
    for num,batch in enumerate(update_batches):
        model = None
        print ("previous model: ", pre_model)
        ...
        x_values, y_values = model.dataset.read_data("../../job_data/"+tr_sets[num], haveheader=True, use_cols=cols,to_dict=True)
        train_set = pd.DataFrame({**x_values, **y_values})
        transfer_data = Dataset.stratified_sample(train_set, model.dataset.x_attributes, size=tr_sizes[num], seed=123, keep_index= True) 
        model.dataset.transfer_set = "data/transfer_set.csv"
        transfer_data.to_csv(model.dataset.transfer_set, sep=model.dataset.delimiter, index=None)
        x_values_transfer, y_values_transfer = model.dataset.read_data(model.dataset.transfer_set, haveheader=True, use_cols=cols, to_dict=True)

        update_file = os.path.join(model.dataset.update_path,batch)
        model.dataset.train_file=update_file
        x_values_update, y_values_update = model.dataset.read_data(update_file, haveheader=True, use_cols=cols, to_dict=True)
        ...
        model.update(x_points_update=x_encoded_update[x_att], y_points_update=y_normalized_update[y_att],
                     ...
                     alpha=alphas[num], gamma=gammas[num], bs=bs[num])
        t2 = time.time()
        print ("Updating finished in {} seconds".format(t2-t1))
        

        out = 'distill'+str(num+1).zfill(2)+'.dill'
        print ("saving at ", out)
        pre_model = out
...
def update_with_finetune(basemodel,cols):
    if not os.path.exists(basemodel):
        print ("Model not found!")
        return
    ...
    for num,batch in enumerate(update_batches):
        ...
        out = 'finetune'+str(num+1).zfill(2)+'.dill'
        pre_model = out
        with open(out,'wb') as d:
            dill.dump(model,d)


def update_FTs(basemodel, cols):
    if not os.path.exists(basemodel):
        print ("Model not found!")
        return
    ...
    for num,batch in enumerate(update_batches):
        ...
        out = 'stale'+str(num+1).zfill(2)+'.dill'
        pre_model = out
        with open(out ,'wb') as d:
            dill.dump(model,d)


def fast_retrain(basemodel, cols):
    files = ['title_until_part2.csv','title_until_part3.csv','title_until_part4.csv','title_until_part5.csv'] 
    #files = ['title_only_part5.csv']
    model = None
    with open(basemodel, 'rb') as d:
        model = dill.load(d)

    for i, file in enumerate(files):
        x_values, y_values = model.dataset.read_data("../../job_data/"+file, haveheader=True, use_cols=cols, to_dict=True, sample=0.01)
      
        model.dataset.create_frequency_tables(data=x_values)
        ...
        with open('fastretrain'+str(i+1).zfill(2)+'.dill','wb') as dum:
            dill.dump(model,dum)


def retrain_all(basemodel, cols):
    #files = ['update01.csv','update02.csv','update03.csv','update04.csv','update05.csv']
    files = ['title_only_part2.csv','title_only_part3.csv','title_only_part4.csv','title_only_part5.csv'] 
    #files = ['title_only_part5.csv']
    model = None
    with open(basemodel, 'rb') as d:
        model = dill.load(d)

    x_values, y_values = model.dataset.read_data('../../job_data/title_until_part1.csv', haveheader=True,use_cols=cols, to_dict=True)
    ...
    for i, file in enumerate(files):
        t1 = time.time()
        x_tmp, y_tmp = model.dataset.read_data('../../job_data/'+file, haveheader=True, use_cols=cols, to_dict=True)
...
if __name__=="__main__":
    #create_ref()
    #train(mode="train", ref='reference.dill', name='base.dill')
    #distilling(basemodel='DenMDN_census.dill')
    #distill_musample(basemodel='DenMDN_census.dill')
    #update_with_fusion(basemodel="base.dill", tr_size=100000, cols=['production_year', 'info_type_id'])
    #update_with_finetune(basemodel="base.dill", cols=['production_year', 'info_type_id'])
    #update_FTs(basemodel="base.dill", cols=['production_year', 'info_type_id'])
    retrain_all(basemodel="base.dill", cols=['production_year', 'info_type_id'])
    #fast_retrain(basemodel="base.dill", cols=['production_year', 'info_type_id'])
