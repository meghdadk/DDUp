...
class DenMDN:
    ...
    def fit(self,x_points, y_points, encoded=True,lr = 0.0001, n_workers=0): 
        ...
        n_hidden_layer = 5#self.config.config["n_hidden_layer"]
...
def create_ref():
    t1 = time.time()
    d = Dataset.Data('../../tpch_data/',train_file=None, test_file=None, update_path=None, transfer_set=None, x_attributes = ["order_date"], y_attributes = ["total_price"], sep=',')
    d.update_path = "../../tpch_data/"
    x_values, y_values = d.read_data("../../tpch_data/orders_until5.csv", haveheader=True, use_cols=['order_date', 'total_price'],to_dict=True)
...
def train(mode, ref, name):
    ...
    t1 = time.time()
    if ref is None:
        d = Dataset.Data('./data/',train_file=None, test_file=None, update_path=None, transfer_set=None, x_attributes = ["order_date"], y_attributes = ["total_price"], sep=',')    
        d.train_update_split(table='tpch_for_MDN.csv', haveheader=True, permute=True, convert_to_timestamp=False, percentage=0.2,num_update_chunks=1)  
    ...
    x_values, y_values = d.read_data("../../tpch_data/orders_until1.csv", haveheader=True, use_cols=['order_date', 'total_price'], to_dict=True)
...
def update_with_finetune(basemodel,cols):
    ...
    update_batches.sort()
    update_batches = ['orders_only2.csv','orders_only3.csv','orders_only4.csv','orders_only5.csv'] 
    print ("Updates order ==>")
    print (update_batches)
...
def fast_retrain(basemodel, cols):
    files = ['orders_only2.csv','orders_only3.csv','orders_only4.csv','orders_only5.csv']
    ...
    for i, file in enumerate(files):
        x_values, y_values = model.dataset.read_data("../../tpch_data/"+file, haveheader=True, use_cols=cols, to_dict=True, sample=0.01)
...
def retrain_all(basemodel, cols):
    ...
    x_values, y_values = model.dataset.read_data('../../tpch_data/orders_only1.csv', haveheader=True,use_cols=cols, to_dict=True)
    ...
    for i, file in enumerate(files):
        t1 = time.time()
        x_tmp, y_tmp = model.dataset.read_data('../../tpch_data/'+file, haveheader=True, use_cols=cols, to_dict=True)
...
if __name__=="__main__":
    ...
    #update_with_finetune(basemodel="base.dill", cols=['order_date', 'total_price'])
    #update_FTs(basemodel="base.dill", cols=['order_date', 'total_price'])
    #retrain_all(basemodel="base.dill", cols=['order_date', 'total_price'])
    fast_retrain(basemodel="base.dill", cols=['order_date', 'total_price'])
