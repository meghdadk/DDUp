...
class Data:
    ...
    def train_update_split(self,table, haveheader, permute, convert_to_timestamp=False, percentage=0.2, num_update_chunks=10):
        ...
        data = self.read_data(path, haveheader, to_dict=False)
        data = data.dropna()
        ...
        train_data.to_csv(self.train_file,sep=self.delimiter, index=None)
        ...
        for i,chunk in enumerate(update_chunks):
            ...
            chunk.to_csv(os.path.join(self.update_path,'update{}.csv'.format(str(i+1).zfill(2))), sep=self.delimiter, index=None)
            #test.to_csv(os.path.join(self.update_path,'update{}_test.csv'.format(str(i+1).zfill(2))), sep=self.delimiter, index=None)

    def read_data(self, file, haveheader, use_cols, to_dict, sample=None):
        if haveheader:
            data = pd.read_csv(file, usecols=use_cols, sep=self.delimiter)
        else:
            if self.header==[]:
                print ("header list is not provided")
                return
            data = pd.read_csv(file, header=None, usecols=use_cols, sep=self.delimiter, names=self.header)
