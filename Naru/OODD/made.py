...
class MADE(nn.Module):
    ...
    def semiparam_kd_loss(self, student_logits, teacher_logits,orig_loss):
        """Calculates -log p(data), given logits (the conditionals).

        """

        loss = KDOrthoLoss()
        kd = torch.zeros(student_logits.size()[0], device=student_logits.device)
        for i in range(self.nin):
            logits1_i = self.logits_for_col(i, student_logits)
            logits2_i = self.logits_for_col(i, teacher_logits)

            target = torch.multinomial(torch.softmax(logits2_i, -1), 1)
            kd += loss.forward(logits1_i, logits2_i, target, loss_original=orig_loss)
