...
def MakeTable(step=None):
    ...
    if args.dataset == 'dmv-tiny':
        ...
        table = datasets.LoadDmv(batch_num=step)
...
def MainAll(modelspath):
    ...
    for step, path in enumerate(models):
        all_ckpts = glob.glob('./{}/{}'.format(modelspath,args.glob))
        if args.blacklist:
            all_ckpts = [ckpt for ckpt in all_ckpts if args.blacklist not in ckpt]
...
def LLTest(modelspath, seed=0):
    ...
    for step, path in enumerate(models):
        ...
        landmarks = int(len(train_data.tuples)*10/12) + np.linspace(0, int(len(train_data.tuples)*0.2), 6, dtype=np.int)
        ...
        #rndindices = torch.randperm(len(train_data.tuples))[:1000000]
...
def test_for_drift(seed=0,bootstrap=1000, sample_size=10000):
