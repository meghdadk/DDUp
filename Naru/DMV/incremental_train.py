...
def RunUpdateEpoch(split,
    ...
    for step, (xb, trb) in enumerate(zip(loader, cycle(trloader))):
        """
        if split == 'train':
            base_lr = 8e-4
            for param_group in opt.param_groups:
            ...
            if split == 'train':
                print(
                    ...
                    .format(epoch_num, step, split,
                            ...
                            loss.item() / np.log(2), table_bits[0], opt.param_groups[0]['lr']))
            else:
                print('Epoch {} Iter {}, {} loss {:.4f} nats / {:.4f} bits'.
                      format(epoch_num, step, split, loss.item(),
                             loss.item() / np.log(2)))
...
def TrainTask(seed=0):
    ...
    if args.heads > 0:
        ...
        if args.dataset in ['dmv-tiny', 'dmv', 'tpcds', 'census']:
            model = MakeMade(
...
def UpdateTask(pre_model, seed=0):
    ...
    for update_step, split_index in enumerate(split_indices):
        table_main = table
        ...
        if args.heads > 0:
            ...
            pmodel = MakeTransformer(cols_to_train=table.columns,
                                    fixed_ordering=fixed_ordering,
                                    seed=seed)
        ...
        if isinstance(model, transformer.Transformer):
            ...
            opt = torch.optim.Adam(list(model.parameters()), lr=5e-4)
            decay_rate = 0.98
            lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(
                optimizer=opt, gamma=decay_rate
            )
        ...
        for epoch in range(args.epochs):
            ...
            mean_epoch_train_loss = RunUpdateEpoch('train',
                                             model,
                                             pmodel,
                                             opt,
                                             scheduler=lr_scheduler,
            ...
            if epoch % 5 ==0:
                torch.save(model.state_dict(), "models/update_batch_{}_epoch_{}.pt".format(update_step+1,epoch))
            train_losses.append(mean_epoch_train_loss)
        ...
        if fixed_ordering is None:
            if seed is not None:
                PATH = 'models/{}{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}.pt'.format(str(update_step+1).zfill(2),
                    args.dataset, mb, model.model_bits, table_bits[0], model.name(),
                    args.epochs, seed)
            else:
                PATH = 'models/{}{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-{}.pt'.format(str(update_step+1).zfill(2),
                    args.dataset, mb, model.model_bits, table_bits[0], model.name(),
            ...
            PATH = 'models/{}{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-order{}{}.pt'.format(str(update_step+1).zfill(2),
                args.dataset, mb, model.model_bits, table_bits[0], model.name(),
                args.epochs, seed, '_'.join(map(str, fixed_ordering)), annot) 
        os.makedirs(os.path.dirname(PATH), exist_ok=True)
        torch.save(model.state_dict(), PATH)
...
def FineTuneTask(pre_model, seed=0):
    ...
    if args.dataset == 'dmv-tiny':
        ...
        table, split_indices = datasets.LoadDmv(batch_num=None, finetune=True)
    elif args.dataset == 'tpcds':
        table, split_indices = datasets.LoadPermutedDataset()
    elif args.dataset == 'census':
        table, split_indices = datasets.LoadCensus()
    ...
    for update_step, split_index in enumerate(split_indices):
        ...
        if isinstance(model, transformer.Transformer):
            ...
            opt = torch.optim.Adam(list(model.parameters()), lr=2e-6)
            decay_rate = 0.98
            lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(
                optimizer=opt, gamma=decay_rate
            )
        ...
        if fixed_ordering is None:
            if seed is not None:
                PATH = 'models/FT{}{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}.pt'.format(str(update_step+1).zfill(2),
                    args.dataset, mb, model.model_bits, table_bits, model.name(),
                    args.epochs, seed)
            else:
                PATH = 'models/FT{}{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-{}.pt'.format(str(update_step+1).zfill(2),
            ...
            PATH = 'models/FT{}{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-order{}{}.pt'.format(str(update_step+1).zfill(2),
                args.dataset, mb, model.model_bits, table_bits[0], model.name(),
                args.epochs, seed, '_'.join(map(str, fixed_ordering)), annot)
        os.makedirs(os.path.dirname(PATH), exist_ok=True)
        torch.save(model.state_dict(), PATH)
...
def DistillTask(pre_model, seed=0):
    ...
    for update_step in range(5):
        ...
        #transfer_data = copy.deepcopy(train_data)
        rndindices = torch.randperm(len(train_data.tuples))[:10000]
        ...
        for epoch in range(args.epochs): 
            ...
            mean_epoch_train_loss = RunDistillEpoch('train',
                                             model,
                                             pmodel,
                                             opt,
                                             scheduler=lr_scheduler,
        ...
        if fixed_ordering is None:
            if seed is not None:
                PATH = 'models/{}distilled{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}.pt'.format(str(update_step+1).zfill(2),
                    args.dataset, mb, model.model_bits, table_bits[0], model.name(),
                    args.epochs, seed)
            else:
                PATH = 'models/{}distilled{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-{}.pt'.format(str(update_step+1).zfill(2),
            ...
            PATH = 'models/{}distilled{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-order{}{}.pt'.format(str(update_step+1).zfill(2),
                args.dataset, mb, model.model_bits, table_bits[0], model.name(),
                args.epochs, seed, '_'.join(map(str, fixed_ordering)), annot)
        os.makedirs(os.path.dirname(PATH), exist_ok=True)
        torch.save(model.state_dict(), PATH)
...
def BayesCardExp(pre_model=None, seed=0):
    ...
    assert args.dataset in ['dmv-tiny', 'dmv','tpcds']
    if args.dataset == 'dmv-tiny':
        table = datasets.LoadDmv('dmv-tiny.csv')
    elif args.dataset == 'dmv':
        table = datasets.LoadDmv()
    ...
    if fixed_ordering is None:
        if seed is not None:
            PATH = 'models/01{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}.pt'.format(args.dataset, mb, model.model_bits, table_bits, model.name(),
                args.epochs, seed)
        else:
            PATH = 'models/01{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-{}.pt'.format(args.dataset, mb, model.model_bits, table_bits, model.name(),
                args.epochs, seed, time.time())
        ...
        PATH = 'models/01{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-order{}{}.pt'.format(args.dataset, mb, model.model_bits, table_bits[0], model.name(),
            args.epochs, seed, '_'.join(map(str, fixed_ordering)), annot) 
    os.makedirs(os.path.dirname(PATH), exist_ok=True)
    torch.save(model.state_dict(), PATH)
    print('Saved to:')
...
#TrainTask()
UpdateTask(pre_model='models/dmv-5.1MB-model19.862-data19.253-made-resmade-hidden256_256_256_256_256-emb32-directIo-binaryInone_hotOut-inputNoEmbIfLeq-colmask-20epochs-seed0.pt')
#FineTuneTask(pre_model='models/dmv-5.1MB-model19.862-data19.253-made-resmade-hidden256_256_256_256_256-emb32-directIo-binaryInone_hotOut-inputNoEmbIfLeq-colmask-20epochs-seed0.pt')
#DistillTask(pre_model='models/dmv-5.1MB-model19.862-data19.253-made-resmade-hidden256_256_256_256_256-emb32-directIo-binaryInone_hotOut-inputNoEmbIfLeq-colmask-20epochs-seed0.pt')
