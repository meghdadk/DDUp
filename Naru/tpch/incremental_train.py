...
def RunUpdateEpoch(split,
    ...
    for step, (xb, trb) in enumerate(zip(loader, cycle(trloader))):
        ...
        if num_orders_to_forward == 1:
            ...
            loss = (1-omega)*(4*loss3.mean()/5 + 1*loss2/5) + omega*loss1
    ...
    if split == "train" and scheduler is not None:
        scheduler.step()
    if return_losses:
        return losses
    return np.mean(losses)
...
def TrainTask(seed=0):
    ...
    if args.dataset == 'dmv-tiny':
        ...
        original = datasets.LoadTPCH(filename='orders_until5.csv')
    ...
    train_data = common.TableDataset(table, original)
    train_losses = []
    ...
    if fixed_ordering is None:
        if seed is not None:
            PATH = 'models/p4retrain{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}.pt'.format(
        ...
        PATH = 'models/p4retrain{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-order{}{}.pt'.format(
            args.dataset, mb, model.model_bits, table_bits, model.name(),
...
def UpdateTask(pre_model, seed=0):
    ...
    if args.dataset == 'dmv-tiny':
        ...
        table, split_indices = datasets.LoadPermutedForest()
    elif args.dataset == 'tpch':
        previous_table = datasets.LoadTPCH(filename='orders_until4.csv')
        table = datasets.LoadTPCH(filename='orders_only5.csv')
    ...
    if args.heads > 0:
        model = MakeTransformer(cols_to_train=reference.columns,
                                fixed_ordering=fixed_ordering,
                                seed=seed)

        pmodel = MakeTransformer(cols_to_train=reference.columns,
                                fixed_ordering=fixed_ordering,
                                seed=seed)
    ...
    transfer_data = common.TableDataset(previous_table, reference)
    rndindices = torch.randperm(len(transfer_data.tuples))[:150000]
    transfer_data.tuples = transfer_data.tuples[rndindices]
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 100 ==0:
            torch.save(model.state_dict(), "models/update_batch_{}_epoch_{}.pt".format(1,epoch))
        train_losses.append(mean_epoch_train_loss)
    ...
    if fixed_ordering is None:
        if seed is not None:
            PATH = 'models/p4ddup{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}.pt'.format(
        ...
        PATH = 'models/p4ddup{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-order{}{}.pt'.format(
            args.dataset, mb, model.model_bits, table_bits[0], model.name(),
            args.epochs, seed, '_'.join(map(str, fixed_ordering)), annot) 
    os.makedirs(os.path.dirname(PATH), exist_ok=True)
    torch.save(model.state_dict(), PATH)
...
def FineTuneTask(pre_model, fast_retrain, seed=0):
    torch.manual_seed(0)
    np.random.seed(0)
    ...
    if args.dataset == 'dmv-tiny':
        ...
        table, split_indices = datasets.LoadCensus(),[48842,87915]
    elif args.dataset == 'forest':
        table, split_indices = datasets.LoadForest(batch_num=None, finetune=True)
    elif args.dataset == 'tpch':
        table = datasets.LoadTPCH(filename='orders_until5.csv')
        reference = datasets.LoadTPCH(filename='orders_until5.csv')
    ...
    train_data = common.TableDataset(table_main, reference)
    if fast_retrain:
        frac = int(0.01 * len(train_data.tuples))
        rndindices = torch.randperm(len(train_data.tuples))[:frac]
        train_data.tuples = train_data.tuples[rndindices]

    train_losses = []
    train_start = time.time()
    print ("finetuning, initial lr = {}".format(opt.param_groups[0]['lr']))
    for epoch in range(args.epochs):
    ...
    if fixed_ordering is None:
        if seed is not None:
            PATH = 'models/p4fastretrain{}{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}.pt'.format(str(1).zfill(2),
                args.dataset, mb, model.model_bits, table_bits, model.name(),
                args.epochs, seed)
        else:
            PATH = 'models/p4fastretrain{}{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-{}.pt'.format(str(1).zfill(2),
        ...
        PATH = 'models/p4fastretrain{}{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-order{}{}.pt'.format(str(1).zfill(2),
            args.dataset, mb, model.model_bits, table_bits[0], model.name(),
            args.epochs, seed, '_'.join(map(str, fixed_ordering)), annot)
    os.makedirs(os.path.dirname(PATH), exist_ok=True)
    torch.save(model.state_dict(), PATH)
...
#TrainTask()
#UpdateTask(pre_model='models/p3dduptpch-4.6MB-model17.875-data15.720-made-resmade-hidden256_256_256_256_256-emb32-directIo-binaryInone_hotOut-inputNoEmbIfLeq-colmask-10epochs-seed0.pt')
FineTuneTask(pre_model='models/p3fastretrain01tpch-4.6MB-model31.703-data17.720-made-resmade-hidden256_256_256_256_256-emb32-directIo-binaryInone_hotOut-inputNoEmbIfLeq-colmask-20epochs-seed0.pt', 
             fast_retrain=True)
