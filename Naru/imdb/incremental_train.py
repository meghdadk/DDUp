...
def RunUpdateEpoch(split,
    ...
    for step, (xb, trb) in enumerate(zip(loader, cycle(trloader))):
        ...
        if num_orders_to_forward == 1:
            ...
            loss = (1-omega)*(2.5*loss3.mean()/5 + 2.5*loss2/5) + omega*loss1
...
def TrainTask(seed=0):
    ...
    if args.dataset == 'dmv-tiny':
        ...
        original = datasets.LoadIMDB(filename='title_until_part5.csv')
    ...
    if fixed_ordering is None:
        if seed is not None:
            PATH = 'models/part4retrain{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}.pt'.format(
        ...
        PATH = 'models/part4retrain{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-order{}{}.pt'.format(
            args.dataset, mb, model.model_bits, table_bits, model.name(),
...
def UpdateTask(pre_model, seed=0):
    ...
    if args.dataset == 'dmv-tiny':
        ...
        previous_table = datasets.LoadIMDB(filename='title_until_part1.csv')
        table = datasets.LoadIMDB(filename='title_only_part2.csv')
    ...
    rndindices = torch.randperm(len(transfer_data.tuples))[:100000]
    ...
    if fixed_ordering is None:
        if seed is not None:
            PATH = 'models/p2ddup{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}.pt'.format(
        ...
        PATH = 'models/p2ddup{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-order{}{}.pt'.format(
            args.dataset, mb, model.model_bits, table_bits[0], model.name(),
            args.epochs, seed, '_'.join(map(str, fixed_ordering)), annot) 
    os.makedirs(os.path.dirname(PATH), exist_ok=True)
    torch.save(model.state_dict(), PATH)
...
def FineTuneTask(pre_model, fast_retrain, seed=0):
    ...
    if fast_retrain:
        frac = int(0.1 * len(train_data.tuples))
    ...
    if fixed_ordering is None:
        if seed is not None:
            PATH = 'models/p5fast{}{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}.pt'.format(str(1).zfill(2),
                args.dataset, mb, model.model_bits, table_bits, model.name(),
                args.epochs, seed)
        else:
            PATH = 'models/p5fast{}{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-{}.pt'.format(str(1).zfill(2),
        ...
        PATH = 'models/p5fast{}{}-{:.1f}MB-model{:.3f}-data{:.3f}-{}-{}epochs-seed{}-order{}{}.pt'.format(str(1).zfill(2),
            args.dataset, mb, model.model_bits, table_bits[0], model.name(),
            args.epochs, seed, '_'.join(map(str, fixed_ordering)), annot)
    os.makedirs(os.path.dirname(PATH), exist_ok=True)
    torch.save(model.state_dict(), PATH)
...
#UpdateTask(pre_model='models/00imdb-2.2MB-model9.274-data9.271-made-resmade-hidden256_256_256_256_256-emb32-directIo-binaryInone_hotOut-inputNoEmbIfLeq-colmask-20epochs-seed0.pt')
FineTuneTask(pre_model='models/p4fast01imdb-2.2MB-model10.104-data10.102-made-resmade-hidden256_256_256_256_256-emb32-directIo-binaryInone_hotOut-inputNoEmbIfLeq-colmask-10epochs-seed0.pt', 
             fast_retrain=True)
